...
export const MyCart = () => {
  ...
  const removeItem = (selectedId: number) => {
    ...
    if (indexToRemove !== -1) {
      const removedQuantity = getQuantity(indexToRemove);
      ...
      setMyCartCounter(Math.max(0, myCartCounter - removedQuantity));
      localStorage.setItem('myCartCounter', Math.max(0, myCartCounter - removedQuantity).toString());

      const updatedTotalPrice =
        updatedProducts.length > 0
          ? updatedProducts.reduce(
            (total, product, i) => total + Number(product.price) * getQuantity(i),
            0
          ).toFixed(2)
          : '0';
  ...
  const incrementQuantity = (productIndex: number) => {
    const currentQuantity = getQuantity(productIndex);

    const updatedQuantity = currentQuantity + 1;
    updateQuantity(productIndex, updatedQuantity);

    const updatedTotalPrice = (Number(totalPrice) + Number(addedProducts[productIndex].price)).toFixed(2);

    updateLocalStorage(productIndex, updatedQuantity, updatedTotalPrice);
  };

  const decrementQuantity = (productIndex: number) => {
    const currentQuantity = getQuantity(productIndex);
  ...
  const updateLocalStorage = (productIndex: number, updatedQuantity: number, updatedTotalPrice: string) => {
    const updatedQuantities = { ...quantities, [productIndex]: updatedQuantity };
    localStorage.setItem('quantities', JSON.stringify(updatedQuantities));
  ...
  const checkoutAuth = authUser ? '/checkout' : '/SignIn'
                      ...
                      <div className='product-card-wrapper'>
                        ...
                        <div className="product-details-container">
                          ...
                          <div className="product-salesprice-container">
                            <p className="product-salesprice cart-info">Basket Amount</p>
                            <p className="product-salesprice-value cart-value">${product.saleprice}</p>
                          </div>
                          <div className="product-salesprice-container">
                            <p className="product-salesprice cart-info">Selection:</p>
                            <p className="product-salesprice-value selection-value">{product.selectedColor} - {product.selectedSize}</p>
                          </div>
                          <div className="product-quantity-container">
                            <p className="product-quantity-label">Qty: {getQuantity(i)}</p>
                            <div className='product-quantity'>
                              <FontAwesomeIcon onClick={() => decrementQuantity(i)} icon={faMinus} />
                              <FontAwesomeIcon onClick={() => incrementQuantity(i)} icon={faPlus} />
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className='product-shopping-container'>
                  <div className='product-shopping-amount-container'>
                    <div className="product-shopping-amount">
                      ...
                      <p className="product-shopping-total">${(addedProducts.reduce((total, product, i) => total + Number(product.price) * getQuantity(i), 0)).toFixed(2)}</p>
                    </div>
                    <Link to={checkoutAuth}>
                      <Button
