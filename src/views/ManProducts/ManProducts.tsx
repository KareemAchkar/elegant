...
export const ManProducts = () => {
  const { menShopProducts, onLoadMore, visibleProducts, selectedCategory, selectedColour, selectedId, setSelectedId, isContainerDisabled } = useContext(ProductsContext);


  const filteredManProducts =
    menShopProducts.filter((product) => {
  ...
  const lastInd = filteredManProducts.length - 1;
      ...
      <div className={cn('container', { 'page-container': isContainerDisabled })}>
        <div className="productlayout-container">
          <div className={cn('products-container', { 'disabled': isContainerDisabled })}>
            {filteredManProducts.slice(0, visibleProducts).map((product, i) => (
              <Product
                product={product}
