...
export const BabyProducts = () => {
  const { babyShopProducts, onLoadMore, visibleProducts, selectedCategory, selectedColour, selectedId, setSelectedId, isContainerDisabled } = useContext(ProductsContext)

  const filteredBabyProducts =
    babyShopProducts.filter((product) => {
  ...
  const lastInd = filteredBabyProducts.length - 1;
      ...
      <div className={cn('container', { 'page-container': isContainerDisabled })}>
        <div className="productlayout-container">
          <div className={cn('products-container', { 'disabled': isContainerDisabled })}>
            {filteredBabyProducts.slice(0, visibleProducts).map((product, i) => (
              <Product
                product={product}
